...
<p align="center">
  ...
  <a href="#day-2-updating-your-provider">Day-2 Updates</a> &middot;
...
- **From weeks to minutes** — Generate a complete, compilable Terraform provider from any OpenAPI spec in a single command
- **Zero-config CRUD detection** — Automatically maps REST endpoints to Terraform resources without manual mapping files
- **AI-assisted analysis** — Optional `--smart` flag uses Claude to resolve ambiguous patterns, detect computed fields, and identify sensitive attributes
- **Safe day-2 updates** — When your API evolves, regenerate only what changed while preserving all your customizations (`_override.go` files are never overwritten)
- **Works on any existing provider** — Run `update` even on hand-written providers — api2tf infers existing resources from the directory structure, no prior state file needed
- **Production-grade output** — Follows HashiCorp's Terraform Plugin Framework, ready for the Terraform and OpenTofu registries
...
## Day-2: Updating Your Provider

APIs evolve — new endpoints appear, fields get added, old resources are deprecated. api2tf is designed for **ongoing maintenance**, not just initial generation. Whether your provider was generated by api2tf or hand-written from scratch, you can use the update workflow to keep it in sync with your API spec.

### Step 1: Preview Changes

Before touching any files, run a dry-run to see exactly what would change:

```bash
docker run --rm -v $(pwd):/workspace -e ANTHROPIC_API_KEY ghcr.io/gofireflyio/api2tf \
  update new-api-spec.yaml ./terraform-provider-myapi/ --smart --dry-run
```

This produces a detailed changelog showing:
- **New resources** detected in the updated spec
- **Updated files** where schemas or CRUD logic changed
- **Removed resources** that no longer appear in the spec (files are NOT deleted — you decide)
- **Skipped files** — your `_override.go` customizations are always preserved
...
```bash
docker run --rm -v $(pwd):/workspace -e ANTHROPIC_API_KEY ghcr.io/gofireflyio/api2tf \
  update new-api-spec.yaml ./terraform-provider-myapi/ --smart
```

### Step 3: Compile and Test

```bash
# Compile the updated provider
docker run --rm -v $(pwd)/terraform-provider-myapi:/app -w /app golang:1.22 \
  sh -c "go mod tidy && go build -o terraform-provider-myapi ./..."
...
### Updating Providers Not Generated by api2tf

api2tf doesn't require a previous `.api2tf.state.json` to work. When you run `update` on an existing provider directory, it **infers the current state** by scanning the Go files in `internal/provider/`:

- Files matching `resource_{name}_schema_gen.go` or `resource_{name}.go` → existing resources
- Files matching `data_source_{name}_schema_gen.go` or `data_source_{name}.go` → existing data sources

This means you can adopt api2tf on any provider at any point — hand-written, generated by another tool, or partially migrated. The first `update` run creates the state file, and subsequent runs use it for precise incremental diffs.

### Comparing Two Spec Versions

To see what changed between two versions of your API spec without modifying any files:

```bash
docker run --rm -v $(pwd):/workspace ghcr.io/gofireflyio/api2tf \
